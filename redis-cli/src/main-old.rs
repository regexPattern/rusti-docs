...
fn main() {
    ...
    let writer_handle = thread::spawn(move || {
        loop {
            ...
            // Usa la función modularizada
            if command_sender::parse_command(&input, &stream_writer) {
                continue;
            }
            ...
            if let Some(cmd) = parts.next() {
                if cmd.eq_ignore_ascii_case("subscribe") {
                    ...
                    let channels: Vec<resp::BulkString> =
                        parts.map(resp::BulkString::from).collect();
                    if !channels.is_empty() {
                        let unsubscribe = commands::pub_sub::Unsubscribe { channels };
                        let cmd = Command::PubSub(unsubscribe.into());
                        let resp_bytes: Vec<u8> = cmd.into();
                        println!(
                            "[DEBUG] Serializado: {:?}",
                            String::from_utf8_lossy(&resp_bytes)
