...
impl StorageCommand {
    pub fn key(&self) -> &BulkString {
        match self {
            StorageCommand::Del(del) => &del.key,
            StorageCommand::HSet(hset) => &hset.key,
            StorageCommand::HGet(hget) => &hget.key,
            StorageCommand::HDel(hdel) => &hdel.key,
            StorageCommand::HGetAll(hget_all) => &hget_all.key,
            StorageCommand::HKeys(hkeys) => &hkeys.key,
            StorageCommand::HVals(hvals) => &hvals.key,
            StorageCommand::HExists(hexists) => &hexists.key,
            StorageCommand::LPush(lpush) => &lpush.key,
            StorageCommand::LInsert(linsert) => &linsert.key,
            StorageCommand::LPop(lpop) => &lpop.key,
            StorageCommand::LIndex(lindex) => &lindex.key,
            StorageCommand::LLen(llen) => &llen.key,
            StorageCommand::LRange(lrange) => &lrange.key,
            StorageCommand::SAdd(sadd) => &sadd.key,
            StorageCommand::SRem(srem) => &srem.key,
            StorageCommand::SCard(scard) => &scard.key,
            StorageCommand::SIsMember(sis_member) => &sis_member.key,
            StorageCommand::SMembers(smembers) => &smembers.key,
            StorageCommand::Set(set) => &set.key,
            StorageCommand::Get(get) => &get.key,
            StorageCommand::Append(append) => &append.key,
            StorageCommand::Decr(decr) => &decr.key,
            StorageCommand::Incr(incr) => &incr.key,
        }
    }
}
